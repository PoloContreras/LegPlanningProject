...
def numericSteps(targetRad):
    if (targetRad >= 3*math.pi/2 and targetRad < 28*math.pi/18) or (targetRad >= math.pi/2 and targetRad < 10*math.pi/18): #ROTATION MANEUVERS
        return [(0,(extremeHip[0],neutralAnkle)),(2,(extremeHip[0],neutralAnkle))] #format: (choice,endpos) with endpos in degrees
    elif (targetRad  > 8*math.pi/18 and targetRad <= math.pi/2) or (targetRad > 26*math.pi/18 and targetRad <= 3*math.pi/2):
        return [(0,(extremeHip[1],neutralAnkle)),(2,(extremeHip[1],neutralAnkle))]
    elif (targetRad >= 0 and targetRad < math.pi/18) or (targetRad >= 2*math.pi and targetRad < 19*math.pi/18):
        return [(1,(extremeHip[0],neutralAnkle)),(3,(extremeHip[0],neutralAnkle))]
    elif (targetRad > 17*math.pi/18 and targetRad <= math.pi) or (targetRad > 35*math.pi/18 and targetRad <= 2*math.pi): 
        return [(1,(extremeHip[1],neutralAnkle)),(3,(extremeHip[1],neutralAnkle))]
    elif targetRad >= 0 and targetRad < math.pi/2: #LOCOMOTION MANEUVERS
