...
import legHardSelector as legSelect
...
theta_start = np.array([[0,0,0,0],[math.radians(30),math.radians(30),-1*math.radians(30),-1*math.radians(30)]])
...
while(not GoalReached):
    ...
    projPoint = legSelect.circularProjection(body_pos[1],legSelect.circle_radius,local_target)
    body_dir = body_frames[1].reshape(3,3)[:2,0]
    angle_proj = positionToAngle(body_pos[1],body_dir,projPoint)
    new_legs = numericSteps(angle_proj)
    ...
    for new_leg in new_legs:
    	...
    	endpos = theta_end  # THESE WERE COLUMN VECTORS IN THE MATLAB CODE
    ...
    for i in range(1500):
        paso = numericWalk(500,i,new_legs)
